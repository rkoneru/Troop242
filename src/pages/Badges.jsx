...
// Static animation variants hoisted to prevent recreation on each render
...
export default function Badges() {
  ...
  // Performance Optimization: Memoize filtered categories to prevent
  // expensive O(N) recalculations on non-search state changes (e.g. category toggle)
    ...
    return BADGE_CATEGORIES
      .map(cat => ({
        ...
        badges: cat.badges.filter(badge =>
          badge.name.toLowerCase().includes(trimmedQuery)
        )
      }))
      .filter(cat =>
        cat.category.toLowerCase().includes(trimmedQuery) ||
        cat.badges.length > 0
      );
  }, [searchTerm]);
