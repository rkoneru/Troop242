...
// Hoisted Framer Motion animation variants to avoid re-allocation on every render/tick
...
// Flip Card Component - Memoized to prevent re-renders on 1s countdown timer ticks in parent
const WhyUsCard = React.memo(function WhyUsCard({ icon: Icon, title, desc }) {
...
// Event Card Component - Memoized so countdown updates only re-render affected cards
const EventCard = React.memo(function EventCard({ event, countdown }) {
  const formattedDate = useMemo(() => {
    return event.date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
  }, [event.date]);
    ...
    <>
      ...
      <section className="section">
        <div className="container">
          ...
          >
            {events.map((event) => (
              <EventCard key={event.id} event={event} countdown={countdowns[event.id]} />
            ))}
          </motion.div>
        </div>
      </section>
