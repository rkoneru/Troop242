...
// Hoisted Framer Motion animation variants to avoid re-creation on render
...
// Helper to calculate countdown for a target date
const calculateCountdownForDate = (targetDate) => {
  const now = new Date();
...
// Memoized EventCard to completely isolate 1-second countdown updates from Home page re-renders
const EventCard = memo(({ event, variants }) => {
  const [countdown, setCountdown] = useState(() => calculateCountdownForDate(event.date));

  useEffect(() => {
    const interval = setInterval(() => {
      setCountdown(calculateCountdownForDate(event.date));
    }, 1000);
    return () => clearInterval(interval);
  }, [event.date]);
