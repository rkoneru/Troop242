              ...
              >
                ...
                <div className="flex" style={{ alignItems: 'flex-start', gap: 16, marginBottom: 24 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ color: '#00d68f', fontSize: '0.9rem', fontWeight: 600 }}>Rank #{i + 1}</p>
                  </div>
                </div>

                {/* Requirements */}
                <div style={{ marginBottom: 24 }}>
                  <h4 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: 12, color: '#00d68f' }}>Requirements</h4>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {rank.requirements.map((req, j) => (
                      <div key={j} className="flex" style={{ gap: 8, fontSize: '0.95rem' }}>
                        <CheckCircle size={16} style={{ color: '#00d68f', flexShrink: 0, marginTop: 2 }} />
                        <span>{req}</span>
                      </div>
                    ))}
                  </div>
                ...
                <div style={{ borderTop: '1px solid rgba(255, 255, 255, 0.1)', paddingTop: 20 }}>
                  <h4 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: 12, color: '#d4a853' }}>Benefits</h4>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {rank.benefits.map((benefit, j) => (
                      <div key={j} style={{ fontSize: '0.95rem', color: '#9ca3af' }}>• {benefit}</div>
                    ))}
                  </div>
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: 16, marginBottom: 40 }}>
                {RANKS.map((rank, i) => (
                  <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                    ...
                    <div style={{ fontSize: '0.85rem', color: '#9ca3af' }}>{rank.name}</div>
                  </div>
                ))}
              </div>
